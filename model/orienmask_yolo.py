...
class OrienMaskYOLO(BaseModel):
    def __init__(self, num_anchors, num_classes, pretrained=None,
        ...
        self.neck4 = self._build_neck(192, 128)

        self.route32 = self._build_route(512)
        self.route16 = self._build_route(256)
        self.route8 = self._build_route(128)
    ...
    @classmethod
    def _build_route(cls, channels):
        return nn.Sequential(
            conv_bn_leaky(channels, channels // 2, 1),
    ...
    def forward(self, x):
        ...
        oriens = self.neck4(torch.cat([self.route8(neck8), x4], dim=1))
