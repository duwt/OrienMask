...
at::Tensor nms_cpu_kernel(const at::Tensor& dets, const float threshold) {
  ...
  auto x_t = dets.select(1, 0).contiguous();
  auto y_t = dets.select(1, 1).contiguous();
  auto w_t = dets.select(1, 2).contiguous();
  auto h_t = dets.select(1, 3).contiguous();
  auto scores = dets.select(1, 4).contiguous();
  auto x1_t = x_t - w_t / 2.0;
  auto y1_t = y_t - h_t / 2.0;
  auto x2_t = x_t + w_t / 2.0;
  auto y2_t = y_t + h_t / 2.0;
...
at::Tensor nms_cpu(const at::Tensor& dets, const float threshold) {
  at::Tensor result;
  AT_DISPATCH_FLOATING_TYPES(dets.type(), "nms", [&] { // dets.scalar_type() got error
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("nms", &nms_cpu, "non-maximum suppression");
}
